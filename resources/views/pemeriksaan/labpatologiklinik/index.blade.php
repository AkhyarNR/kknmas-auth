  ...
  <div class="">
    ...
    <div class="row">
      <div class="col-md-12 col-sm-12 ">
        <div class="x_panel">
          <div class="x_title">
            <h3>Antrian Lab.Patologi Klinik</h3>
            <div class="clearfix"></div>
          </div>
          <div class="x_content">
            <div id="grid"></div>
...
<script>
  ...
  $(function(){
    ...
    $('#grid').kendoGrid({
      ...
      dataSource: {
        transport:{
          read : function(options) {
            ...
            $.ajax({
              url : '{{ url('pemeriksaan/labpatologiklinik/read') }}',
              type: 'GET',
              dataType: 'json',
              data: options.data,
      ...
      columns:[
        ...
        {field: 'Dokter_Id', title: 'Nama Dokter',template:"#if(data.Full_Name == null){# - #}else{# #=data.Full_Name# #}#"},
                    ...
                    actions: [
                        {
                            ...
                            action: function (e) {
                                ...
                                $.ajax({
                                    ...
                                    url: "{{ asset('pemeriksaan/labpatologiklinik/hapus') }}",
                                    type: "GET",
                                    data: id,
                                    dataType: "json",
                                    complete: function (e) {
  ...
  function Periksa(e) {
    e.preventDefault();
        ...
        data = this.dataItem(tr);
        console.log(data)
        var id = {Antrian_Pasien_Id: data.Antrian_Pasien_Id, Pasien_Id : data.Pasien_Id, Work_Unit_Id : data.Work_Unit_Id,Dokter_Id : data.Dokter_Id, Shift_Id : data.Shift_Id, Pasien_Lab_Id : data.Pasien_Lab_Id, Jenis_Pemeriksaan_Id:data.Jenis_Pemeriksaan_Id};

            $.ajax({
                headers: {
                    "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content")
                },
                url: "{{ asset('pemeriksaan/labpatologiklinik/create') }}",
                type: "POST",
                data: id,
                dataType: "json",
                success : function(res){
                          window.open('{{ url("pemeriksaan/patologiklinik") }}/'+res,'_self');
                  }
              });
  }
    ...
    e.preventDefault();
        ...
        var id = {Antrian_Pasien_Id: data.Antrian_Pasien_Id, Pasien_Id : data.Pasien_Id, Work_Unit_Id : data.Work_Unit_Id,Dokter_Id : data.Dokter_Id, Shift_Id : data.Shift_Id};
            ...
            $.ajax({
                ...
                url: "{{ asset('pemeriksaan/labpatologiklinik/skip') }}",
                type: "POST",
                data: id,
                dataType: "json",
                success : function(res){
  ...
  function detailInit(e) {
      ...
      $.ajax({
              url : '{{ url('pemeriksaan/labpatologiklinik/alergipasien') }}',
              dataType:'json',
              type:'GET',
      ...
      $.ajax({
              url : '{{ url('pemeriksaan/labpatologiklinik/asuransipasien') }}',
              dataType:'json',
              type:'GET',
              data:{'Pasien_Id' : data.Pasien_Id},
              success:function(res){
